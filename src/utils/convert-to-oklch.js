export function convertToOklch({ color, shade = "500", transparency = "100" }) {
  ...
  if (shade == 500 && transparency == 100) return `var(--${color})`;

  const alpha = transparency < 100 ? transparency / 100 : 1;

  if (shade == 500) return `oklch(from var(--${color}) l c h / ${alpha})`;

  let ratio = -0.2 * shade + 100;
  let baseColor = `color-mix(in oklch, var(--${color}), #fff ${ratio}%)`;

  if (shade > 500) {
    ratio = 0.2 * shade - 100;
    baseColor = `color-mix(in oklch, var(--${color}), #000 ${ratio}%)`;
  } 
